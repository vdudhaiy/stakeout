...
def _load_ticker_df(ticker: str, days: int) -> pd.DataFrame:
   '''
   Load the archived OHLCV CSV for `ticker` and return a DataFrame trimmed to
   the last `days` rows, sorted ascending by date.

   Steps:
   1. Build the CSV path from ARCHIVE_DATA_DIR and the uppercased ticker symbol
      (match the exact filename convention used by the pipeline).
   2. Read the CSV with pandas.  The date column should become the index.
   3. Sort ascending so that row 0 is the oldest date.
   4. Slice to the last `days` rows.  If days <= 0, return the full history —
      some indicators (RSI, MACD) need more lookback than the user-facing window
      to "warm up" the calculation; see the note in each compute_* function.
   5. Raise ValueError with a descriptive message if the file does not exist,
      so the router can convert it to a 404.
   '''
   files = os.listdir(ARCHIVE_DATA_DIR)
   ticker_file = sorted(f for f in files if f.startswith(ticker) and f.endswith(".csv"))

   if not ticker_file:
       raise ValueError(f"Data for ticker '{ticker}' not found.")

   file_path = os.path.join(ARCHIVE_DATA_DIR, ticker_file[-1])
...
async def fetch_indicators(
   ...
   df = _load_ticker_df(ticker, days + warmup_buffer)
   sma_list = [
