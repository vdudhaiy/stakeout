...
Mocks filesystem (os.listdir / os.remove / pd.read_csv), yfinance (yf.Ticker),
and pandas_market_calendars (mcal) at the call site instead of at the router
boundary so that the transformation / parsing logic inside each function is
actually executed and covered.
"""
...
async def test_get_all_stocks_reads_archive_and_builds_dict():
    ...
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["AAPL_2020-01-01_2024-01-16.csv"]):
...
async def test_get_all_stocks_cache_hit_skips_listdir():
    _snapshot_cache.set("all_stocks", {"AAPL": "Apple"})

    with patch("market_lens_dashboard.services.stock_service.os.listdir") as mock_ls:
        result = await get_all_stocks()
...
async def test_get_all_stocks_empty_dir_returns_empty_dict():
    with patch("market_lens_dashboard.services.stock_service.os.listdir", return_value=[]):
        result = await get_all_stocks()
    assert result == {}


async def test_get_all_stocks_ignores_non_csv_files():
    mock_ticker = MagicMock()
    mock_ticker.ticker = "AAPL"
    mock_ticker.info = {"displayName": "Apple Inc."}

    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["AAPL_2020-01-01_2024-01-16.csv", "README.md", ".DS_Store"]):
        with patch("market_lens_dashboard.services.stock_service.yf.Ticker", return_value=mock_ticker):
            result = await get_all_stocks()
...
async def test_delete_stock_removes_matching_files():
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["AAPL_2020-01-01_2024-01-16.csv", "MSFT_2020-01-01_2024-01-16.csv"]):
        with patch("market_lens_dashboard.services.stock_service.os.remove") as mock_rm:
            result = await delete_stock("AAPL")

    assert "deleted successfully" in result["message"]
    assert mock_rm.call_count == 1
    assert "AAPL" in str(mock_rm.call_args[0][0])


async def test_delete_stock_no_matching_files_raises():
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["MSFT_2020-01-01_2024-01-16.csv"]):
        with pytest.raises(ValueError, match="No CSV data found"):
...
async def test_delete_stock_invalidates_cache():
    ...
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["AAPL_2020-01-01_2024-01-16.csv"]):
        with patch("market_lens_dashboard.services.stock_service.os.remove"):
            await delete_stock("AAPL")
...
async def test_fetch_returns_ohlcv_response():
    ...
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["AAPL_2020-01-01_2024-01-16.csv"]):
        with patch("market_lens_dashboard.services.stock_service.pd.read_csv", return_value=df):
            with patch(
...
async def test_fetch_no_csv_files_raises():
    with patch("market_lens_dashboard.services.stock_service.os.listdir", return_value=[]):
        with pytest.raises(ValueError, match="No CSV data found"):
            await fetch("AAPL")


async def test_fetch_all_nan_close_raises():
    df = pd.DataFrame([{
        "date": "2024-01-15", "open": 183.0, "high": 185.0,
        "low": 182.0, "close": float("nan"), "volume": 5_000_000,
    }])
    ...
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
        ...
        with patch("market_lens_dashboard.services.stock_service.pd.read_csv", return_value=df):
            with patch(
                ...
                with pytest.raises(ValueError, match="No confirmed OHLCV data"):
                    await fetch("AAPL")
...
async def test_fetch_stale_data_triggers_append():
    ...
    with patch("market_lens_dashboard.services.stock_service.os.listdir",
               return_value=["AAPL_2020-01-01_2024-01-11.csv"]):
        with patch("market_lens_dashboard.services.stock_service.pd.read_csv", return_value=df):
            with patch(
                "market_lens_dashboard.services.stock_service._last_completed_trading_day",
                new_callable=AsyncMock,
                ...
                with patch("market_lens_pipeline.fetchers.price.append_price_data") as mock_append:
