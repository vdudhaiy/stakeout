...
import { House, LayoutDashboard, Briefcase, Sun, Moon, RefreshCw, ChevronDown, LogOut, UserRound } from 'lucide-react'
...
const IN_SESSIONS = [
  { label: 'Pre-open', color: 'text-amber-400',   window: '09:00 – 09:15 IST' },
  { label: 'Regular',  color: 'text-emerald-400', window: '09:15 – 15:30 IST' },
  { label: 'Closing',  color: 'text-blue-400',    window: '15:40 – 16:00 IST' },
] as const
...
export function Navbar({
  ...
  const { currency, setCurrency, usdInr } = usePrefs()
  const { enabled: authOn, user, signOut } = useAuth()
  ...
  useEffect(() => {
    const popups: Array<[boolean, React.RefObject<HTMLDivElement>, (v: boolean) => void]> = [
      ...
      [showCurrency, currencyRef, setShowCurrency],
  ...
  }, [showMarketPopup, showHealthPopup, showCurrency, showUser])
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <div className="relative" ref={currencyRef}>
          <button
            ...
            className={clsx(
              'flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-mono font-medium transition-colors',
              showCurrency
          ...
          {showCurrency && (
            <div className="absolute right-0 top-full mt-2 z-50 w-56 bg-zinc-950 border border-zinc-700 rounded-xl p-2 shadow-2xl">
              {(['USD', 'INR'] as const).map(c => (
                <button
                  ...
                  className={clsx(
                    ...
                    currency === c ? 'bg-indigo-500/10 text-indigo-300' : 'text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200',
                  )}
                >
                  <span>{c === 'USD' ? '$ US Dollar' : '₹ Indian Rupee'}</span>
                  {currency === c && <span className="text-[9px]">✓</span>}
                </button>
              ))}
              <div className="mt-1.5 pt-2 border-t border-zinc-800 px-3 pb-1 flex items-center gap-1.5 text-[10px] text-zinc-600 font-mono">
                <span>1 USD ≈ {usdInr ? `₹${usdInr.toFixed(2)}` : '—'}</span>
                <InfoTip k="fx_rate" align="right" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative" ref={healthRef}>
          ...
          {showHealthPopup && (
            <div className="absolute right-0 top-full mt-2 z-50 w-72 bg-zinc-950 border border-zinc-700 rounded-xl p-4 shadow-2xl">
              ...
              <p className="mt-3 pt-3 border-t border-zinc-800 text-[10px] text-zinc-600">Auto-checks every 30 s</p>
            </div>
          )}
        </div>
        ...
        <div className="relative" ref={marketRef}>
          ...
          {showMarketPopup && (
            <div className="absolute right-0 top-full mt-2 z-50 w-80 bg-zinc-950 border border-zinc-700 rounded-xl p-4 shadow-2xl">
              ...
              <div className="space-y-2">
                {IN_SESSIONS.map(({ label, color, window: w }) => (
                  <div key={label} className="flex items-center justify-between text-xs font-mono">
                    <span className={clsx('text-[10px] font-semibold tracking-widest', color)}>{label.toUpperCase()}</span>
              ...
              <p className="mt-3 pt-3 border-t border-zinc-800 text-[10px] text-zinc-600">Auto-refreshes every 5 min</p>
            </div>
          )}
        </div>
