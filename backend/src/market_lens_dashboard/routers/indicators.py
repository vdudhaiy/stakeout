...
async def get_sma(ticker: str, days: int = 90, period: int = 20):
    ...
    '''
    try:
        df = await indicators_service._load_ticker_df(ticker, days + period)
...
async def get_rsi(ticker: str, days: int = 90, period: int = 14):
    ...
    '''
    try:
        df = await indicators_service._load_ticker_df(ticker, days + period + 1)
...
async def get_macd(
    ...
    '''
    try:
        df = await indicators_service._load_ticker_df(ticker, days + slow + signal)
