...
async def test_sync_inserts_dividends_after_earliest_buy(aapl_session):
    ...
    with patch("services.portfolio_service.yf.Ticker") as mock_ticker:
        ...
        result = await portfolio_service.sync_dividends(aapl_session, USER_ID, "AAPL")
...
async def test_sync_skips_ex_dates_before_earliest_buy(aapl_session):
...
async def test_sync_never_overwrites_existing_row(aapl_session):
    """A manually-edited or auto-fetched row for a date is never clobbered by a later sync."""
    await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.99"))
...
async def test_sync_is_throttled_within_24h(aapl_session):
    series = _dividend_series({"2024-03-01": 0.24})
    with patch("services.portfolio_service.yf.Ticker") as mock_ticker:
        mock_ticker.return_value.dividends = series
        await portfolio_service.sync_dividends(aapl_session, USER_ID, "AAPL")
...
async def test_sync_skips_dates_with_zero_shares_held(aapl_session):
    ...
    with patch("services.portfolio_service._current_price",
        ...
        await portfolio_service.sell_stock_shares(
            aapl_session, USER_ID, "AAPL", shares=100, sold_at=Decimal("200.0"), date="2024-02-01"
        )
...
async def test_sync_unknown_ticker_raises(db_session):
    with pytest.raises(ValueError, match="No holding"):
        await portfolio_service.sync_dividends(db_session, USER_ID, "NOTEXIST")


# ── add_dividend / update_dividend / delete_dividend ─────────────────────────

async def test_add_dividend_defaults_shares_held_from_fifo_log(aapl_session):
    entry = await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"))
    assert entry.shares_held == 100
    assert float(entry.total_amount) == pytest.approx(50.0)
    assert entry.source == "manual"


async def test_add_dividend_explicit_shares_held_overrides_fifo(aapl_session):
    entry = await portfolio_service.add_dividend(
        aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"), shares_held=40,
    )
    assert entry.shares_held == 40
    assert float(entry.total_amount) == pytest.approx(20.0)


async def test_add_dividend_duplicate_date_raises(aapl_session):
    await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"))
    with pytest.raises(ValueError, match="already exists"):
        await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.6"))


async def test_add_dividend_future_date_raises(aapl_session):
    import datetime
    future = (datetime.date.today() + datetime.timedelta(days=5)).isoformat()
    with pytest.raises(ValueError, match="future"):
        await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", future, Decimal("0.5"))


async def test_add_dividend_before_earliest_buy_raises(aapl_session):
    with pytest.raises(ValueError, match="before the earliest purchase"):
        await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2023-12-01", Decimal("0.5"))


async def test_add_dividend_with_no_shares_held_raises(aapl_session):
    with pytest.raises(ValueError, match="No shares"):
        await portfolio_service.add_dividend(
            aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"), shares_held=0,
        )


async def test_update_dividend_recomputes_total_and_marks_manual(aapl_session):
    entry = await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"))
    updated = await portfolio_service.update_dividend(
        aapl_session, USER_ID, "AAPL", entry.id, amount_per_share=Decimal("1.0"),
    )
    assert float(updated.total_amount) == pytest.approx(100.0)
    assert updated.source == "manual"


async def test_update_dividend_not_found_raises(aapl_session):
    with pytest.raises(ValueError, match="not found"):
        await portfolio_service.update_dividend(aapl_session, USER_ID, "AAPL", 9999, amount_per_share=Decimal("1.0"))


async def test_delete_dividend_removes_it(aapl_session):
    entry = await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"))
    await portfolio_service.delete_dividend(aapl_session, USER_ID, "AAPL", entry.id)
    assert await portfolio_service.get_dividends(aapl_session, USER_ID, "AAPL") == []


async def test_delete_dividend_not_found_raises(aapl_session):
    with pytest.raises(ValueError, match="not found"):
        await portfolio_service.delete_dividend(aapl_session, USER_ID, "AAPL", 9999)


# ── roll-up into StockHolding / PortfolioResponse ─────────────────────────────

async def test_stock_holding_includes_total_dividends(aapl_session):
    await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"))
...
async def test_portfolio_net_pl_includes_dividends(aapl_session):
    await portfolio_service.add_dividend(aapl_session, USER_ID, "AAPL", "2024-03-01", Decimal("0.5"))
...
async def test_new_holding_purchase_schedules_dividend_sync(db_session):
