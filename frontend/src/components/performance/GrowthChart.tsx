...
function GrowthTooltip({ active, payload, benchmarkName }: any) {
 ...
 * user's savings rate.
 */
export function GrowthChart({ points, days, benchmarkName }: Props) {
  if (points.length < 2) return null

  const ticks = thinTicks(computeXTicks(points.map(p => p.date), days), MAX_TICKS)
  const all = points.flatMap(p => [p.portfolio_index, p.benchmark_index])
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={points} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
        ...
        />
        <Tooltip content={<GrowthTooltip benchmarkName={benchmarkName} />} />
