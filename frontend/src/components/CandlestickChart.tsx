...
function makeTooltip(overlays: OverlayConfig, currencySymbol: string) {
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-3 shadow-xl text-xs font-mono">
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-1">
          ...
          <span className="text-zinc-200">{currencySymbol}{(d.open as number | null)?.toFixed(2) ?? '—'}</span>
          <span className="text-zinc-500">H</span>
          <span className="text-emerald-400">{currencySymbol}{(d.high as number | null)?.toFixed(2) ?? '—'}</span>
          <span className="text-zinc-500">L</span>
          <span className="text-red-400">{currencySymbol}{(d.low as number | null)?.toFixed(2) ?? '—'}</span>
          <span className="text-zinc-500">C</span>
          <span className="text-zinc-100 font-medium">{currencySymbol}{(d.close as number).toFixed(2)}</span>
        </div>
        {hasOverlays && (
            ...
            {overlays.activeSMA.map(p => {
              ...
              return val != null ? (
                <div key={p} className="flex justify-between gap-4">
                  ...
                  <span className="text-zinc-300">{currencySymbol}{val.toFixed(2)}</span>
                </div>
              ) : null
            })}
...
export function CandlestickChart({ data, days, overlays, currencySymbol = '$' }: Props) {
  const tooltipContent = useMemo(() => makeTooltip(overlays, currencySymbol), [overlays, currencySymbol])
