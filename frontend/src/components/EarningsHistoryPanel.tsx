...
function makeFmtRevenue(currencySymbol: string) {
  return (v: number | null | undefined): string => {
    if (v == null) return '—'
    const abs = Math.abs(v)
    if (abs >= 1e12) return `${currencySymbol}${(v / 1e12).toFixed(2)}T`
    if (abs >= 1e9) return `${currencySymbol}${(v / 1e9).toFixed(2)}B`
    if (abs >= 1e6) return `${currencySymbol}${(v / 1e6).toFixed(2)}M`
...
function RevenueChart({ rows, currencySymbol }: { rows: RevenueHistoryRow[]; currencySymbol: string }) {
  const [mode, setMode] = useState<'growth' | 'actual'>('growth')
  const fmtRevenue = makeFmtRevenue(currencySymbol)
...
export function EarningsHistoryPanel({ epsHistory, revenueHistory, currencySymbol = '$' }: Props) {
