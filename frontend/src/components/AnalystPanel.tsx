...
function makeFmtPrice(currencySymbol: string) {
...
function makeFmtBig(currencySymbol: string) {
  return (n: number | null | undefined) => {
    if (n == null) return '—'
    const abs = Math.abs(n)
    if (abs >= 1e12) return `${currencySymbol}${(n / 1e12).toFixed(2)}T`
    if (abs >= 1e9) return `${currencySymbol}${(n / 1e9).toFixed(2)}B`
    if (abs >= 1e6) return `${currencySymbol}${(n / 1e6).toFixed(2)}M`
...
function PriceTargetsCard({ targets, currentPrice, format, currencySymbol }: PriceTargetsProps) {
  const money = format ?? makeFmtPrice(currencySymbol)
...
function EarningsEstimatesCard({ estimates, currencySymbol }: EarningsEstimatesProps) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
      ...
      {rows.length === 0 ? (
        ...
        <EstimatesTable
          rows={rows}
          fmtValue={makeFmtPrice(currencySymbol)}
          valueLabel="AVG EPS"
        />
      )}
    </div>
...
function RevenueEstimatesCard({ estimates, currencySymbol }: RevenueEstimatesProps) {
  const rows = (estimates ?? []).filter(r => r.avg != null)
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
      ...
      {rows.length === 0 ? (
        ...
        <EstimatesTable
          rows={rows}
          fmtValue={makeFmtBig(currencySymbol)}
          valueLabel="AVG"
        />
      )}
    </div>
